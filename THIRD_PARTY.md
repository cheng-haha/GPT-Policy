...
`scripts/setup_robodojo.sh` installs XPolicyLab's WebSocket dependencies into
the same `.venv` and generates a `GPT_Policy` wrapper under the ignored
XPolicyLab checkout. A protocol-level server/client smoke test can therefore
run without Isaac Sim; the actual task client additionally requires RoboDojo's
Isaac Sim/Isaac Lab/CUDA installation.
