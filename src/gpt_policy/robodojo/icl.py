...
class RoboDojoICL:
    def __init__(self, root: str | os.PathLike[str], cache_dir: str | os.PathLike[str], *, keyframes: int = 6) -> None:
    ...
    def prepare(self, instruction: str) -> tuple[str, dict[str, Path]] | None:
        ...
        summary = self._summary(task, instruction, rows, positions, times)
        return summary, image_paths

    @staticmethod
    def _summary(task: str, instruction: str, rows: list[dict[str, Any]], positions: list[int], times: list[float]) -> str:
        samples = []
        for index, timestamp in zip(positions, times):
            row = rows[index]
            action = row.get("actions.eef") or row.get("action")
            state = row.get("observation.states.eef") or row.get("observation.state")
            samples.append({"t_s": round(timestamp, 3), "state_eef": _round_list(state), "action_eef": _round_list(action)})
        return json.dumps({
